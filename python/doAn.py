...
def conv(img, kernel):
    ...
    if len(img.shape) != len(kernel.shape) - 1: # Check whether number of dimensions is the same
        print("Error: Number of dimensions in con v filter and image do not match.")  
        exit()
    if len(img.shape) > 2 or len(kernel.shape) > 3: # Check if number of image channels matches the filter depth.
        if img.shape[-1] != kernel.shape[-1]:
            print("Error: Number of channels in both image and filter must match.")
            exit()
    if kernel.shape[1] != kernel.shape[2]: # Check if filter dimensions are equal.
    ...
    for filter_num in range(kernel.shape[0]):
        ...
        curr_filter = kernel[filter_num, :] # getting a filter from the bank.
        """ 
...
def relu(feature_map):
    ...
    for map_num in range(feature_map.shape[-1]):  #map_num chinh la số layer của 1 bức ảnh
        for r in numpy.arange(0,feature_map.shape[0]):
            for c in numpy.arange(0, feature_map.shape[1]):
                relu_out[r, c, map_num] = numpy.max([feature_map[r, c, map_num], 0])  # = a ? a>0 : = 0
    return relu_out
